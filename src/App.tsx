...
function App() {
  ...
  const [isEformsMenuOpen, setIsEformsMenuOpen] = useState(false);
  ...
  const [openAcordForm, setOpenAcordForm] = useState<EformsAction | null>(null);
    ...
    <div className="app-shell">
      ...
      <div className="workspace">
        <aside className="sidebar">
          ...
          <div className="side-group bottom-actions"><div className="side-label" title="Form and document tools"><Settings size={13} /> Actions</div><button className={`side-item ${isEformsMenuOpen ? 'selected' : ''}`} onClick={() => setIsEformsMenuOpen((current) => !current)} title="Open the eForms quick menu"><Files size={13} /> eForms</button>{isEformsMenuOpen && <div className="eforms-menu"><button onClick={() => { setIsEformsMenuOpen(false); setIsEformsOpen(true); setEformsTab('All Forms'); }} title="Open the full eForms Manager window">Launch eForms Manager</button><strong>New</strong>{['Applications', 'Auto ID Card', 'Binder', 'Cancellation', 'Certificate of Liability', 'Certificate of Property', 'Change Request', 'EPI', 'Loss Notice', 'Additional Forms'].map((item) => <button key={item} onClick={() => { setIsEformsMenuOpen(false); setEformsTab(item === 'Certificate of Liability' ? 'Certificates' : 'All Forms'); setIsEformsOpen(true); }} title={`Start a new ${item} form`}>{item}</button>)}</div>}<button className="side-item quick" title="Run a saved quick report"><Files size={13} /> Quick Reports</button></div>
        </aside>
...
type EformsAction = 'new-cert-lib' | 'new-cert-prop' | 'acord-125' | 'acord-126' | 'acord-127' | 'acord-130' | 'acord-131' | 'acord-140';

const eformsTiles: Record<string, { label: string; icon: typeof FileBadge; action?: EformsAction }[]> = {
  'All Forms': [{ label: 'ACORD 125', icon: FileBadge, action: 'acord-125' }, { label: 'ACORD 126', icon: FileBadge, action: 'acord-126' }, { label: 'ACORD 127', icon: FileBadge, action: 'acord-127' }, { label: 'ACORD 25', icon: ShieldCheck }, { label: 'Auto ID Card', icon: FileBadge }, { label: 'Binder', icon: FileBadge }, { label: 'Cancellation', icon: FileBadge }, { label: 'Change Request', icon: FileBadge }, { label: 'EPI', icon: FileBadge }, { label: 'Loss Notice', icon: FileBadge }],
  'Applications': [{ label: 'ACORD 125', icon: FileBadge, action: 'acord-125' }, { label: 'ACORD 126', icon: FileBadge, action: 'acord-126' }, { label: 'ACORD 127', icon: FileBadge, action: 'acord-127' }, { label: 'ACORD 130', icon: FileBadge, action: 'acord-130' }, { label: 'ACORD 131', icon: FileBadge, action: 'acord-131' }, { label: 'ACORD 140', icon: FileBadge, action: 'acord-140' }],
...
function EformsManager({ customer, tab, onTabChange, onClose, onNewCertificate, onOpenAcordForm }: { customer: Customer | null; tab: string; onTabChange: (tab: string) => void; onClose: () => void; onNewCertificate: () => void; onOpenAcordForm: (form: EformsAction) => void }) {
  const tiles = eformsTiles[tab] || eformsTiles['All Forms'];
  function handleTileClick(tile: { label: string; action?: EformsAction }): void {
...
function CertificateDetail({ certificate, customer, holders, onClose, onNotify, onManageHolders }: { certificate: Certificate; customer: Customer | null; holders: CertificateHolder[]; onClose: () => void; onNotify: (message: string) => void; onManageHolders: () => void }) {
  ...
  return <div className="modal-backdrop"><section className="acord-drawer"><div className="modal-header"><div><span className="eyebrow">Certificate register</span><h2>{certificate.certificate_number}</h2></div><button onClick={onClose} aria-label="Close" title="Close this preview"><X size={19} /></button></div><div className="detail-status"><span className="pill green" title="Certificate status">{certificate.status}</span><span title="Date this certificate was issued">Issued {new Date(certificate.issued_date).toLocaleDateString('en-US')}</span><button className="link-button" onClick={onManageHolders} title="Add or edit who this certificate names as holder">Manage Holders</button></div>
    <div className="acord-body">
      ...
      <p className="acord-boilerplate">THIS IS TO CERTIFY THAT THE POLICIES OF INSURANCE LISTED BELOW HAVE BEEN ISSUED TO THE INSURED NAMED ABOVE FOR THE POLICY PERIOD INDICATED. NOTWITHSTANDING ANY REQUIREMENT, TERM OR CONDITION OF ANY CONTRACT OR OTHER DOCUMENT WITH RESPECT TO WHICH THIS CERTIFICATE MAY BE ISSUED OR MAY PERTAIN, THE INSURANCE AFFORDED BY THE POLICIES DESCRIBED HEREIN IS SUBJECT TO ALL THE TERMS, EXCLUSIONS AND CONDITIONS OF SUCH POLICIES. LIMITS SHOWN MAY HAVE BEEN REDUCED BY PAID CLAIMS.</p>
      <table className="acord-coverage-table"><thead><tr><th title="Letter referencing which insurer wrote this coverage">INSR LTR</th><th title="Coverage line">TYPE OF INSURANCE</th><th title="Additional insured?">ADDL INSD</th><th title="Subrogation waived?">SUBR WVD</th><th title="Policy number for this coverage">POLICY NUMBER</th><th title="Policy effective date">POLICY EFF</th><th title="Policy expiration date">POLICY EXP</th><th title="Coverage limits">LIMITS</th></tr></thead><tbody>
        <tr><td>A</td><td>COMMERCIAL GENERAL LIABILITY<br />☐ CLAIMS-MADE ☒ OCCUR<br />GEN'L AGGREGATE LIMIT APPLIES PER: ☒ POLICY ☐ PROJECT ☐ LOC</td><td>{holder ? 'Y' : ''}</td><td>{holder ? 'Y' : ''}</td><td>{certificate.policy_number}</td><td>{effDate}</td><td>{expDate}</td><td>EACH OCCURRENCE $1,000,000<br />DAMAGE TO RENTED PREMISES (Ea occurrence) $100,000<br />MED EXP (Any one person) $5,000<br />PERSONAL &amp; ADV INJURY $1,000,000<br />GENERAL AGGREGATE $2,000,000<br />PRODUCTS - COMP/OP AGG $2,000,000</td></tr>
        <tr><td>A</td><td>AUTOMOBILE LIABILITY<br />☒ ANY AUTO ☐ OWNED AUTOS ONLY<br />☐ SCHEDULED AUTOS ☐ HIRED AUTOS ONLY ☐ NON-OWNED AUTOS ONLY</td><td>{holder ? 'Y' : ''}</td><td>{holder ? 'Y' : ''}</td><td>{certificate.policy_number}</td><td>{effDate}</td><td>{expDate}</td><td>COMBINED SINGLE LIMIT (Ea accident) $1,000,000<br />BODILY INJURY (Per person) $<br />BODILY INJURY (Per accident) $<br />PROPERTY DAMAGE (Per accident) $</td></tr>
        <tr><td /><td>☐ UMBRELLA LIAB ☐ OCCUR<br />☐ EXCESS LIAB ☐ CLAIMS-MADE<br />☐ DED ☐ RETENTION $</td><td /><td /><td /><td /><td /><td>EACH OCCURRENCE $<br />AGGREGATE $</td></tr>
        <tr><td /><td>WORKERS COMPENSATION<br />AND EMPLOYERS' LIABILITY<br />ANY PROPRIETOR/PARTNER/EXECUTIVE<br />OFFICER/MEMBER EXCLUDED? (Mandatory in NH) — N/A</td><td /><td /><td /><td /><td /><td>PER STATUTE ☐ OTH-ER ☐<br />E.L. EACH ACCIDENT $<br />E.L. DISEASE - EA EMPLOYEE $<br />E.L. DISEASE - POLICY LIMIT $</td></tr>
        <tr><td /><td>OTHER:</td><td /><td /><td /><td /><td /><td /></tr>
      </tbody></table>
